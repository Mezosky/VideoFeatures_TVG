...
def get_annotations(path1: str, path2: str, path3: str) -> None:
    ...
    with open("/data/imeza/annotations/anet/annotations/train_i3d.json") as f:
        json_train = json.load(f)
        json_train = json_train["annotations"]

    # test file created by Cristian
    with open("/data/imeza/annotations/anet/annotations/valid_i3d.json") as f:
    ...
    for it, ann_file in enumerate(list_annotations_files):
        ...
        if it == 0:
            new_json_name = f"anet_train_tokens_{model_name}.json"
        else:
            new_json_name = f"anet_test_tokens_{model_name}.json"
